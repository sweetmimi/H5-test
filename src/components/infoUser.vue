...
export default {
  ...
  async created() {
    ...
    this.amount=amount
    try {
      this.Doevent('info_personal_view');
    } catch (e) {
    }
  },
  mounted() {
  ...
  methods: {
    ...
    //输入监控操作
    // async blurFun(item) {
    //   let info = await this.axios.post(this.$IPS.apifield,
    ...
    async toselect(item) {
      ...
      this.num++
       //let urlArr = ['/loan-dana/province', '/loan-dana/city', '/loan-dana/district', '/loan-dana/small-district']
      //地址接口
    ...
    async toSubmit() {
      ...
      try {
        this.Doevent('info_personal_next_click');
      } catch (e) {
      }
      let data = {
        order: {},
