      ...
      <div v-for="(item,index) in infoList"
        ...
        <div class="mgt10 info-div"
          ...
          <input v-else
                 ...
                 :placeholder="item.example_val ">
          <i
            v-if="item.check_type=='phone'||item.check_type=='telephone_or_phone'||item.check_type=='disabled_input_phone'"
            @click="toselectTel(index)"
            class="icon-right-addtel iconfont icon-yuanquanjiahao"></i>
          <div class="br"></div>
        </div>
        ...
        <div class="mgt10 info-div"
          ...
          <em class="address-style"
              v-else>{{item[item.code].text.provinces}} {{item[item.code].text.city}} {{item[item.code].text.districts}}
            {{item[item.code].text.villages}}</em>
          <i class="icon-right iconfont icon-youjiantou"></i>
          <div class="br"></div>
        </div>
...
<script>
  ...
  export default {
    ...
    this.amount=amount
      try {
        this.Doevent('info_contack_view');
      } catch (e) {
      }
    },
    mounted() {
  ...
  window["comebackIndex"] =()=>{
    ...
    methods: {
      ...
      //输入监控操作
      // async blurFun(item) {
      //   let info = await this.axios.post(this.$Api.apifield, {
      //     pro_code: 4000,
      ...
      async toSubmit() {
        ...
        try {
          this.Doevent('info_contack_next_click');
        } catch (e) {
        }
        let data = {
         order: {},
